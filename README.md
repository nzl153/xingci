...
- 鼠标移上去：翻前后、点喇叭读一遍、开关自动朗读、调间隔；下面的「记得 / 忘了」和卡片上的评分一样算进进度
- 双击打开完整卡片；拖动可以挪位置
- 不抢焦点，不会吃掉你的键盘输入；超过 3 分钟没碰键盘鼠标或者锁屏了就不换词
- 托盘菜单：开关浮窗、暂停一会儿、调间隔、自动朗读、开机自启。`Alt+Shift+W` 唤出主窗口
...
- **桌面版**：进度存在项目根目录的 `save/` 文件夹里，都是普通的 JSON 文件（进度、文章、浮窗历史），每个文件都留有上一版的 `.bak`。备份就是把这个文件夹拷走。如果项目放在写不进去的地方（比如 Program Files），会改存到用户目录（Windows 上是 `%APPDATA%\星词\save`）。
...
| `core.js` | 主窗口和浮窗共用：存档、抽词权重、释义截短、发音 |
| `desktop/` | Electron 壳：托盘、浮窗、存档读写、发音缓存 |
| `tools/build_dict.py` | 从 ECDICT 生成 `data/` |

桌面版带一套自检，会在临时目录里起一次完整的桌面版，逐项检查浮窗、评分、存档、发音、视图切换，不碰你的进度：
